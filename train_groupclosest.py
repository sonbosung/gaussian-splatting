...
def training(dataset, 
    ...
    if enable_visualization:
        ...
        # 모든 학습 시점에 대해 시각화
        for i, uid in enumerate(scheduler.uid_to_name.keys()):
            if i % 25 == 0:  # 5x5 그리드로 나누기
            ...
            # 현재 학습 시점을 초록색으로 표시
            current_idx = uid_to_idx[uid]
            plt.scatter(cam_center_2d[current_idx, 0], cam_center_2d[current_idx, 1],
                       c='green', s=200)
            
            # 인접 시점들을 파란색으로 표시
            current_name = scheduler.uid_to_name[uid]
            closest_names = scheduler.closest_n_views[current_name]
            closest_uids = [scheduler.name_to_uid[name] for name in closest_names]
            closest_indices = [uid_to_idx[u] for u in closest_uids]
            plt.scatter(cam_center_2d[closest_indices, 0], cam_center_2d[closest_indices, 1],
                       c='blue', s=150, alpha=0.7)
            
            plt.title(f'View {i+1}')
            
            if (i + 1) % 25 == 0 or i == len(scheduler.uid_to_name) - 1:
                plt.tight_layout()
                plt.savefig(os.path.join(vis_dir, f'views_{i//25 + 1}.png'))
                plt.close()
