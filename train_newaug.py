...
def evaluate_test_images(scene, render_func, render_args, iteration, tb_writer=None):
    ...
    Args:
        scene: Scene 객체
        render_func: 렌더링 함수
        render_args: 렌더링 함수에 전달할 추가 인자
        iteration: 현재 iteration
        tb_writer: TensorBoard 기록용 객체 (선택적)
    ...
    for idx, camera in enumerate(test_cameras):
        ...
        if FUSED_SSIM_AVAILABLE:
            ssim_value = fused_ssim(rendered_image.unsqueeze(0), gt_image.unsqueeze(0)).item()
        else:
            ssim_value = ssim(rendered_image, gt_image).item()
            ssim_value = ssim_value.mean().item() if isinstance(ssim_value, torch.Tensor) else ssim_value.item()

        total_psnr += psnr_value
        total_ssim += ssim_value

        # TensorBoard에 기록 (최대 5개의 이미지만 기록)
        if tb_writer and idx < 5:
            tb_writer.add_images(f"test_images/rendered_{idx}", rendered_image.unsqueeze(0), global_step=iteration)
            tb_writer.add_images(f"test_images/ground_truth_{idx}", gt_image.unsqueeze(0), global_step=iteration)
