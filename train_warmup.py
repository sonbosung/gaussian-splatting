...
def training(dataset, 
    ...
    if bundle_training:
        ...
        scheduler = PartialGroupScheduler(cameras, clustering.ordered_cluster_names,
                                   ...
                                   bb = bb,
                                   debug = False)
        scheduler.assign_ImageClustering(clustering)

    progress_bar = tqdm(range(first_iter, opt.iterations), desc="Training progress")
    first_iter += 1
    if bundle_training and enable_visualization:
        # 카메라 위치 시각화를 위한 초기 설정
        cam_center_dict = {}
        rotations = {}
        translations = {}
        
        # 각 카메라의 rotation과 translation 계산
        for cam in cameras:
            R = cam.R  # 이미 numpy array입니다
            T = cam.T  # 이미 numpy array입니다
            rotations[cam.uid] = R
            translations[cam.uid] = T
            cam_center_dict[cam.uid] = (-R.T @ T.reshape(3,1))
        ...
        # uid to index mapping 생성
        uid_to_idx = {cam.uid: idx for idx, cam in enumerate(cameras)}

        # 시각화를 위한 디렉토리 생성
        vis_dir = os.path.join(dataset.model_path, 'view_visualization')
    ...
    for iteration in range(first_iter, opt.iterations + 1):
        ...
        if bundle_training:
            ...
            if bundle_training and iteration <= 500 and enable_visualization:
                plt.figure(figsize=(10, 10))
                
                # 모든 카메라 위치 표시
                plt.scatter(cam_center_2d[:, 0], cam_center_2d[:, 1], 
                           c='gray', s=100, alpha=0.3)
                
                current_idx = uid_to_idx[vind]
                
                # 홀수 iteration에서는 선택된 view와 예상되는 least similar view를 미리 표시
                if iteration % 2 == 1:
                    # 현재 선택된 view
                    plt.scatter(cam_center_2d[current_idx, 0], cam_center_2d[current_idx, 1],
                               c='red', s=200, label='Selected View')
                    
                    # 이 view에 대한 least similar view 계산
                    expected_least_similar_name = scheduler.clustering.find_least_similar_view(scheduler.uid_to_name[vind])
                    expected_least_similar_uid = scheduler.name_to_uid[expected_least_similar_name]
                    expected_idx = uid_to_idx[expected_least_similar_uid]
                    
                    # 예상되는 least similar view를 초록색으로 표시
                    plt.scatter(cam_center_2d[expected_idx, 0], cam_center_2d[expected_idx, 1],
                               c='green', s=200, marker='x', label='Expected Least Similar View')
                    plt.plot([cam_center_2d[current_idx, 0], cam_center_2d[expected_idx, 0]],
                            [cam_center_2d[current_idx, 1], cam_center_2d[expected_idx, 1]],
                            'g--', alpha=0.5)
                    
                    # 다음 iteration 비교를 위해 저장
                    prev_vind = vind
                    expected_next_vind = expected_least_similar_uid
                    
                # 짝수 iteration에서는 이전 예상과 실제 선택된 view 비교
                else:
                    prev_idx = uid_to_idx[prev_vind]
                    # 이전 view
                    plt.scatter(cam_center_2d[prev_idx, 0], cam_center_2d[prev_idx, 1],
                               c='red', s=200, label='Previous View')
                    
                    # 예상했던 least similar view
                    expected_idx = uid_to_idx[expected_next_vind]
                    plt.scatter(cam_center_2d[expected_idx, 0], cam_center_2d[expected_idx, 1],
                               c='green', s=200, marker='x', label='Expected View')
                    
                    # 실제 선택된 view
                    plt.scatter(cam_center_2d[current_idx, 0], cam_center_2d[current_idx, 1],
                               c='blue', s=200, marker='x', label='Actual Selected View')
                    
                    # 연결선 표시
                    plt.plot([cam_center_2d[prev_idx, 0], cam_center_2d[expected_idx, 0]],
                            [cam_center_2d[prev_idx, 1], cam_center_2d[expected_idx, 1]],
                            'g--', alpha=0.5, label='Expected Connection')
                    plt.plot([cam_center_2d[prev_idx, 0], cam_center_2d[current_idx, 0]],
                            [cam_center_2d[prev_idx, 1], cam_center_2d[current_idx, 1]],
                            'b--', alpha=0.5, label='Actual Connection')
                
                plt.title(f'View Selection at Iteration {iteration}\nRed: Selected, Green: Expected, Blue: Actual')
                plt.legend()
                plt.savefig(os.path.join(vis_dir, f'view_selection_{iteration:04d}.png'))
                plt.close()
                
                # 현재 view를 다음 iteration을 위해 저장
                prev_vind = vind
